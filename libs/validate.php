...
class Validate{
	...
	public function run(){
		foreach($this->rules as $element => $value){
			if($value['required'] == true && trim($this->source[$element])==null){
				$this->setError($element, 'Gía trị nhập vào không được rỗng!');
			}else{
				switch ($value['type']) {
					case 'int':
					...
					case 'email':
						...
						break;
					case 'status':
						$this->validateStatus($element, $value);
						break;
					case 'group':
					...
					case 'file':
						...
						break;
					case 'login':
						$this->validateLogin($element, $value['options']);
						break;
	...
	private function validateInt($element, $min, $max){
		...
		if(!filter_var($this->source[$element], FILTER_VALIDATE_INT, array("options"=>array("min_range"=>$min,"max_range"=>$max)))){
			if (!filter_var($int, FILTER_VALIDATE_INT) === 0 || filter_var($int, FILTER_VALIDATE_INT)) {
			    $this->setError($element, 'Giá trị không hợp lệ!');
			}
	...
	private function validateString($element, $min = 0, $max = 0){
		...
		if($length < $min) {
			$this->setError($element, 'Gía trị nhập vào quá ngắn!');
		}elseif($length > $max){
			$this->setError($element, 'Gía trị nhập vào quá dài!');
		}elseif(!is_string($this->source[$element])){
			$this->setError($element, 'Gía trị nhập vào không hợp lệ!');
		}
	}
	...
	private function validateStatus($element, $options){
		...
		if(in_array($this->source[$element], $options['options']['deny']) == true){	
			$this->setError($element, 'Vui lòng chọn giá trị');
	...
	private function validatePassword($element, $options){	
		...
		if($options['action'] == 'add' || ($options['action'] == 'edit' && $this->source[$element] )){
			...
			if(!preg_match($pattern, $this->source[$element])){
				$this->setError($element, 'Mật khẩu nhập vào không hợp lệ! ');
			};
		}
	...
	private function validateExistRecord($element, $options){
		...
		if($database->isExist($query)==true){
			$this->setError($element, 'Dữ liệu đã tồn tại!');

		}
	}

	// Validate login
	public function validateLogin($model, $query){
		if($model->isExist($query)==false){
			$this->setError('wanning', 'Tên đăng nhập hặc mật khẩu không chính xác!');
			return false;
		}
		return true;
	}
	...
	private function validateFile($element, $options){
			// 		echo '<pre>';
			// echo print_r($this->source[$element]);
			// echo '<pre>';
			// die();
		if($this->source[$element]['name'] != null){ // kiểm tra ảnh mới có dc thêm hay ko

<?php 	
	class BookController extends Controller{
		...
		public function IndexAction(){
			...
			//selectbox group
			$ỉtemselectbox = $this->_model->categorySelectbox($this->_arrParam);
		...
		public function AddAction(){
			...
			// echo '<pre>';die();
				...
				$validate->addRule('name', 'string',array('min'=>3, 'max'=>255))
						->addRule('price', 'int',array('min'=>100000, 'max'=>1000000))
						->addRule('sale_off', 'int',array('min'=>0, 'max'=>50))
						 ...
						 ->addRule('name','existRecord', array('database'=>$this->_model, 'query'=>"SELECT * FROM `book` WHERE name = '$name'"))
				...
				if($validate->isValid()==false){
					...
					if($this->_arrParam['type']=='save'){
						$this->_model->saveBook($this->_arrParam);
			...
			$this->_view->arrParam = $this->_arrParam;
			$this->_view->render('book/add', true);
		...
		public function EditAction(){
			...
			// echo '<pre>';die();
				...
				$validate->addRule('name', 'string',array('min'=>3, 'max'=>255))
						->addRule('price', 'int',array('min'=>100000, 'max'=>1000000))
						->addRule('sale_off', 'int',array('min'=>-1, 'max'=>50))
						 ...
						 ->addRule('status', 'status',array('deny'=>array('default')))	/// nếu giá trị nhâp vào thuôc mảng deny thì báo lỗi
						 ->addRule('category_id', 'status',array('deny'=>array('default')))
						 ->addRule('name','existRecord', array('database'=>$this->_model, 'query'=>"SELECT * FROM `book` WHERE name = '$name' AND id !=$id"))
						 ->addRule('picture', 'file',array('min'=>100, 'max'=>1000000, 'extension'=>array('jpg', 'png')), false);
